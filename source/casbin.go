...
var casbines = []gormadapter.CasbinRule{
	// ACL 模型: p, sub, obj, act
...
func (c *casbin) Init() error {
	_ = global.GVA_DB.AutoMigrate(gormadapter.CasbinRule{})
	return global.GVA_DB.Transaction(func(tx *gorm.DB) error {
		if tx.Find(&[]gormadapter.CasbinRule{}).RowsAffected == 109 {
			color.Danger.Println("\n[Mysql] --> casbin_rule 表的初始数据已存在!")
			return nil
		}
		if err := tx.Create(&casbines).Error; err != nil {
			return err
		}
